...
class ListSpec extends MultipleCases {
  ...
  test("appendAll") {
    assertEquals(List(1, 2, 3).appendAll(List(4, 5, 6)), List(1, 2, 3, 4, 5, 6))
  }

  test("concatenate") {
    assertEquals(List.concatenate(List(List(1, 2, 3), List(4, 5, 6), List(7, 8, 9))), List(1, 2, 3, 4, 5, 6, 7, 8, 9))
  }
}
